...
/*  Customize these to limit each variable to narrowest possible set of known good values. 
  ...
  $imagesetid = substr(preg_replace('/[^A-Za-z0-9\.\.\ \[NULL\]]/','',$_GET['imagesetid']), 0, 20);
*/

   //---------------------------------------------------------------------------

   private $collectionobjectid; // PK INTEGER 
   private $imagesetid; // PK BIGINT 
   private $dirty;
   private $loaded;
   private $error;
   const FIELDLIST = ' collectionobjectid, imagesetid, ';
   const PKFIELDLIST = ' collectionobjectid, imagesetid, ';
   const NUMBER_OF_PRIMARY_KEYS = 2;
   private $primaryKeyArray = array( 1 => 'collectionobjectid' , 2 => 'imagesetid'  ) ;
   private $allFieldsArray = array( 0 => 'collectionobjectid' , 1 => 'imagesetid'  ) ;
   private $selectDistinctFieldsArray = array( 0 => 'collectionobjectid' , 1 => 'imagesetid'  ) ;

   //---------------------------------------------------------------------------

   // constructor 
   function huh_IMAGE_SET_collectionobject(){
   ...
   public function keyValueSet($fieldname,$value) {
       ...
       if ($this->hasField($fieldname)) { 
          try {
             ...
             if ($fieldname=='imagesetid') { $returnvalue = $this->setimagesetid($value); } 
             $returnvalue = true;
          }
          catch (exception $e) { ;
              $returnvalue = false;
   ...
   public function keyGet($fieldname) {
       ...
       if ($this->hasField($fieldname)) { 
          try {
             ...
             if ($fieldname=='imagesetid') { $returnvalue = $this->getimagesetid(); } 
          }
          catch (exception $e) { ;
              $returnvalue = null;
          }
...
/*collectionobjectid*/
   ...
   public function setcollectionobjectid($collectionobjectid) {
       if (strlen(preg_replace('/[^0-9]/','',$collectionobjectid)) > huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID_SIZE) { 
...
/*imagesetid*/
   public function getimagesetid() {
       if ($this->imagesetid==null) { 
          ...
          return trim($this->l_stripslashes($this->imagesetid));
       }
   }
   public function setimagesetid($imagesetid) {
       if (strlen($imagesetid) > huh_IMAGE_SET_collectionobject::IMAGESETID_SIZE) { 
           throw new Exception('Value exceeds field length.');
       } 
       $this->imagesetid = $this->l_addslashes($imagesetid);
       $this->dirty = true;
   }
   ...
   public function PK() { // get value of primary keys as comma separated list of values 
        ...
        $comma = ', ';
        $returnvalue .= $comma . $this->getimagesetid();
        $comma = ', ';
        return $returnvalue;
   }
   public function PKArray() { // get name and value of primary key fields 
        ...
        $returnvalue['imagesetid'] = $this->getimagesetid();
        return $returnvalue;
   }
  ...
  const C_imagesetidMYSQLI_TYPE = 'i';
   ...
   public function MySQLiFieldType($aFieldname) { 
      ...
      if ($aFieldname=='imagesetid') { $retval = self::C_imagesetidMYSQLI_TYPE; }
      return $retval;
   }
   ...
   public function load($pk) {
        ...
        try {
             ...
             $this->setimagesetid($pk[imagesetid]);
             } else { ;
        ...
        if(collectionobjectid != NULL  && imagesetid != NULL ) {
           $sql = 'SELECT collectionobjectid, imagesetid FROM IMAGE_SET_collectionobject WHERE collectionobjectid = "'. $this->collectionobjectid.'"  and imagesetid = "'. $this->imagesetid.'" ' ;

           $preparesql = 'SELECT collectionobjectid, imagesetid FROM IMAGE_SET_collectionobject WHERE collectionobjectid = ? imagesetid = ?  ';
           $values = array('pk0' => $this->collectionobjectid);           $values = array('pk1' => $this->imagesetid);
           if ($statement = $connection->prepare($preparesql)) { 
              $statement->bind_param("ii", $this->collectionobjectid, $this->imagesetid);
              $statement->execute();
              $statement->bind_result($this->collectionobjectid, $this->imagesetid);
              $statement->fetch();
              $statement->close();
           }
   ...
   public function save($doinsert=false) {
        ...
        global $connection;
        $returnvalue = false;
        if ($doinsert) {            $sql  = 'UPDATE  IMAGE_SET_collectionobject SET ';
            $isInsert = false;
            ...
            $sql  = 'INSERT INTO IMAGE_SET_collectionobject ';
            $isInsert = true;
            $sql .= '( collectionobjectid ,  imagesetid ) VALUES (';
            $sql .=  "  ? ";
            $sql .=  " ,  ? ";
            $sql .= ')';
        ...
        if ($statement = $connection->prepare($sql)) { 
           if ($this->collectionobjectid!= NULL & $this->imagesetid!= NULL ) {
              $statement->bind_param("iiii", $this->collectionobjectid , $this->imagesetid , $this->collectionobjectid , $this->imagesetid );
           } else { 
              $statement->bind_param("ii", $this->collectionobjectid , $this->imagesetid );
           } 
           $statement->execute();
   ...
   public function delete() {
        ...
        if(collectionobjectid != NULL  && imagesetid != NULL ) {
           ...
           $preparedsql = 'SELECT  FROM IMAGE_SET_collectionobject WHERE  and collectionobjectid = ?  and imagesetid = ?  ' ;
        if ($statement = $connection->prepare($preparedsql)) { 
           $statement->bind_param("ii", $this->collectionobjectid, $this->imagesetid);
           $statement->execute();
           $statement->store_result();
           if ($statement->num_rows()==1) {
                $sql = 'DELETE FROM IMAGE_SET_collectionobject WHERE  and collectionobjectid = ?  and imagesetid = ?  ';
                if ($stmt_delete = $connection->prepare($sql)) { 
                   $stmt_delete->bind_param("ii", $this->collectionobjectid, $this->imagesetid);
   ...
   public function count() {
        ...
        global $connection;
        $returnvalue = false;
        $sql = 'SELECT count(*)  FROM IMAGE_SET_collectionobject';
   ...
   public function loadArrayKeyValueSearch($searchTermArray) {
       ...
       $sql = "SELECT collectionobjectid, imagesetid FROM IMAGE_SET_collectionobject $wherebit";
       if ($wherebit=='') { 
          ...
          while ($statement->fetch()) {
              $obj = new huh_IMAGE_SET_collectionobject();
...
// TODO: *************** link to related tables 
   ...
   public function keySelectAllConcatJSON($orderby='ASC') {
       ...
       $sql = "SELECT collectionobjectid, imagesetid, concat() FROM IMAGE_SET_collectionobject order by collectionobjectid, imagesetid $order ";
       $comma = '';
       if ($result = $connection->query($sql)) { 
   ...
   public function keySelectDistinctJSON($field,$orderby='ASC') {
       ...
       if ($this->hasField($field)) { 
          ...
          if ($orderby=='ASC') { $order = 'ASC'; } else { $order = 'DESC'; } 
          $preparemysql = "SELECT DISTINCT $fieldesc FROM IMAGE_SET_collectionobject order by $fieldesc $order ";
          $comma = '';
   ...
   public function loadArrayBycollectionobjectid($searchTerm) {
        ...
        if (!(strpos($searchTerm,"%")===false)) { $operator = "like"; }
        if (!(strpos($searchTerm,"_")===false)) { $operator = "like"; }
        $sql = "SELECT collectionobjectid = :pk0 , imagesetid = :pk1  FROM IMAGE_SET_collectionobject WHERE collectionobjectid $operator '$searchTerm'";
        $preparedsql = "SELECT collectionobjectid = :pk0 , imagesetid = :pk1  FROM IMAGE_SET_collectionobject WHERE collectionobjectid $operator ? ";
        if ($statement = $connection->prepare($preparedsql)) { 
   ...
   public function loadArrayByimagesetid($searchTerm) {
        // ******* Note: $connection must be a mysqli object.
        global $connection;
        $returnvalue = array();
        $operator = "=";
        ...
        if (!(strpos($searchTerm,"%")===false)) { $operator = "like"; }
        if (!(strpos($searchTerm,"_")===false)) { $operator = "like"; }
        $sql = "SELECT collectionobjectid = :pk0 , imagesetid = :pk1  FROM IMAGE_SET_collectionobject WHERE imagesetid $operator '$searchTerm'";
        $preparedsql = "SELECT collectionobjectid = :pk0 , imagesetid = :pk1  FROM IMAGE_SET_collectionobject WHERE imagesetid $operator ? ";
        if ($statement = $connection->prepare($preparedsql)) { 
            $statement->bind_param("s", $searchTerm);
            $statement->execute();
            $statement->bind_result($id);
   ...
   public function selectDistinctcollectionobjectid($startline,$link,$endline,$includecount=false,$orderbycount=false) {
        ...
        $sql = "SELECT count(*) as druid_ct, collectionobjectid FROM IMAGE_SET_collectionobject group by collectionobjectid order by $order ";
        if ($result = $connection->query($sql)) { 
           while ($row = $result->fetch_row()) {
              if ($row) {
                  $count = $row[0];
   ...
   public function selectDistinctimagesetid($startline,$link,$endline,$includecount=false,$orderbycount=false) {
        // ******* Note: $connection must be a mysqli object.
        global $connection;
        $returnvalue = '';
        $order = ' imagesetid ';
        if ($orderbycount) { $order = ' druid_ct DESC '; } 
        $sql = "SELECT count(*) as druid_ct, imagesetid FROM IMAGE_SET_collectionobject group by imagesetid order by $order ";
        if ($result = $connection->query($sql)) { 
           while ($row = $result->fetch_row()) {
              if ($row) {
                  $count = $row[0];
                  ...
                  if ($link=='') { 
                     ...
                     $returnvalue .= "$startline<a href='$link&imagesetid=$escaped'>$val</a>&nbsp;($count)$endline";
                  }
              }
           }
           $result->close();
   ...
   public function keySelectDistinct($fieldname,$startline,$link,$endline,$includecount=false,$orderbycount=false) {
       ...
       switch ($fieldname) { 
          case 'collectionobjectid':
             ...
             break;
          case 'imagesetid':
             $returnvalue = $this->selectDistinctimagesetid($startline,$link,$endline,$includecount,$orderbycount);
             break;
       }
       return $returnvalue;
    }
...
// can use Druid to regenerate the IMAGE_SET_collectionobject.php file to reflect changes
// in the underlying database without overwriting your custom views of the data.
// 
class huh_IMAGE_SET_collectionobjectView implements viewer
{
   var $model = null;
   ...
   public function getDetailsView($includeRelated=true, $editLinkURL='') {
       ...
       if ($editLinkURL!='') { 
          ...
          if (!$nullpk) { $returnvalue .= "<li>huh_IMAGE_SET_collectionobject <a href='$editLinkURL'>Edit</a></li>\n";  } 
       }
       $returnvalue .= "<li>".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID.": ".$model->getcollectionobjectid()."</li>\n";
       $returnvalue .= "<li>".huh_IMAGE_SET_collectionobject::IMAGESETID.": ".$model->getimagesetid()."</li>\n";
       if ($includeRelated) { 
   ...
   public function getJSON() {
       $returnvalue = '{ ';
       $model = $this->model;
       $returnvalue .= '"'.huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID.': "'.$model->getcollectionobjectid().'",';
       $returnvalue .= '"'.huh_IMAGE_SET_collectionobject::IMAGESETID.': "'.$model->getimagesetid().'" }';
       $returnvalue .= '</ul>';
       return  $returnvalue;
   }
   public function getTableRowView() {
       ...
       $returnvalue .= "<td>".$model->getimagesetid()."</td>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
   public function getHeaderRow() {
       $returnvalue = '<tr>';
       $returnvalue .= "<th>".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID."</th>\n";
       $returnvalue .= "<th>".huh_IMAGE_SET_collectionobject::IMAGESETID."</th>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
  ...
  function save'.$id.'() {
     ...
     dojo.xhrGet(saveprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_IMAGE_SET_collectionobject">';
       $returnvalue .= "<div id='div_$formnane' >";
       $returnvalue .= "<div><label for=".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID.">COLLECTIONOBJECTID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID." id=".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID." value='".$model->getcollectionobjectid()."'  style=' width:".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID_SIZE ."em;  '  maxlength='".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_IMAGE_SET_collectionobject::IMAGESETID.">IMAGESETID</label><input  dojoType='dijit.form.ValidationTextBox'  name=".huh_IMAGE_SET_collectionobject::IMAGESETID." id=".huh_IMAGE_SET_collectionobject::IMAGESETID." value='".$model->getimagesetid()."'  style=' width:".huh_IMAGE_SET_collectionobject::IMAGESETID_SIZE ."em;  '  maxlength='".huh_IMAGE_SET_collectionobject::IMAGESETID_SIZE ."' ></div>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
  ...
  function deleterecord'.$id.'() {
     ...
     dojo.xhrGet(deleteprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_table value="huh_IMAGE_SET_collectionobject">';
       $returnvalue .= '<input type=hidden name=collectionobjectid, imagesetid id=druid_table value="'.$id.'">';
       $returnvalue .= '<ul>';
   ...
   public function getEditFormView($includeRelated=true) {
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_IMAGE_SET_collectionobject">';
       $returnvalue .= '<ul>';
       $returnvalue .= "<li>COLLECTIONOBJECTID<input type=text name=".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID." id=".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID." value='".$model->getcollectionobjectid()."'  size='".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID_SIZE ."'  maxlength='".huh_IMAGE_SET_collectionobject::COLLECTIONOBJECTID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>IMAGESETID<input type=text name=".huh_IMAGE_SET_collectionobject::IMAGESETID." id=".huh_IMAGE_SET_collectionobject::IMAGESETID." value='".$model->getimagesetid()."'  size='".huh_IMAGE_SET_collectionobject::IMAGESETID_SIZE ."'  maxlength='".huh_IMAGE_SET_collectionobject::IMAGESETID_SIZE ."' ></li>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
