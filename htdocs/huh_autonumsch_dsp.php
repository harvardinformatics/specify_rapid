...
class huh_autonumsch_dsp implements  model , loadableModel, saveableModel, tableSchema 
{
...
*/
   ...
   private $loaded;
   private $error;
   const FIELDLIST = ' DisciplineID, AutoNumberingSchemeID, ';
   const PKFIELDLIST = ' DisciplineID, AutoNumberingSchemeID, ';
   const NUMBER_OF_PRIMARY_KEYS = 2;
   private $primaryKeyArray = array( 1 => 'DisciplineID' , 2 => 'AutoNumberingSchemeID'  ) ;
   private $allFieldsArray = array( 0 => 'DisciplineID' , 1 => 'AutoNumberingSchemeID'  ) ;
   private $selectDistinctFieldsArray = array(  ) ;

   //---------------------------------------------------------------------------

   // constructor 
   function huh_autonumsch_dsp(){
...
/*DisciplineID*/
   ...
   public function setDisciplineID($DisciplineID) {
       if (strlen(preg_replace('/[^0-9]/','',$DisciplineID)) > huh_autonumsch_dsp::DISCIPLINEID_SIZE) { 
...
/*AutoNumberingSchemeID*/
   ...
   public function setAutoNumberingSchemeID($AutoNumberingSchemeID) {
       if (strlen(preg_replace('/[^0-9]/','',$AutoNumberingSchemeID)) > huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
  ...
  const C_AutoNumberingSchemeIDMYSQLI_TYPE = 'i';
   ...
   public function load($pk) {
        ...
        if(DisciplineID != NULL  && AutoNumberingSchemeID != NULL ) {
           $sql = 'SELECT DisciplineID, AutoNumberingSchemeID FROM autonumsch_dsp WHERE DisciplineID = "'. $this->DisciplineID.'"  and AutoNumberingSchemeID = "'. $this->AutoNumberingSchemeID.'" ' ;

           $preparesql = 'SELECT DisciplineID, AutoNumberingSchemeID FROM autonumsch_dsp WHERE DisciplineID = ? AutoNumberingSchemeID = ?  ';
           $values = array('pk0' => $this->DisciplineID);           $values = array('pk1' => $this->AutoNumberingSchemeID);
           if ($statement = $connection->prepare($preparesql)) { 
              $statement->bind_param("ii", $this->DisciplineID, $this->AutoNumberingSchemeID);
              $statement->execute();
              $statement->bind_result($this->DisciplineID, $this->AutoNumberingSchemeID);
              $statement->fetch();
              $statement->close();
           }
   ...
   public function save($doinsert=false) {
        ...
        global $connection;
        $returnvalue = false;
        if ($doinsert) {            $sql  = 'UPDATE  autonumsch_dsp SET ';
            $isInsert = false;
            $sql .=  "DisciplineID = ? ";
            ...
            $sql  = 'INSERT INTO autonumsch_dsp ';
            $isInsert = true;
            $sql .= '( DisciplineID ,  AutoNumberingSchemeID ) VALUES (';
            $sql .=  "  ? ";
            $sql .=  " ,  ? ";
            $sql .= ')';
        ...
        if ($statement = $connection->prepare($sql)) { 
           if ($this->DisciplineID!= NULL & $this->AutoNumberingSchemeID!= NULL ) {
              $statement->bind_param("iiii", $this->DisciplineID , $this->AutoNumberingSchemeID , $this->DisciplineID , $this->AutoNumberingSchemeID );
           } else { 
              $statement->bind_param("ii", $this->DisciplineID , $this->AutoNumberingSchemeID );
           } 
           $statement->execute();
   ...
   public function delete() {
        ...
        if(DisciplineID != NULL  && AutoNumberingSchemeID != NULL ) {
           ...
           $preparedsql = 'SELECT  FROM autonumsch_dsp WHERE  and DisciplineID = ?  and AutoNumberingSchemeID = ?  ' ;
        if ($statement = $connection->prepare($preparedsql)) { 
           $statement->bind_param("ii", $this->DisciplineID, $this->AutoNumberingSchemeID);
           $statement->execute();
           $statement->store_result();
           if ($statement->num_rows()==1) {
                $sql = 'DELETE FROM autonumsch_dsp WHERE  and DisciplineID = ?  and AutoNumberingSchemeID = ?  ';
                if ($stmt_delete = $connection->prepare($sql)) { 
                   $stmt_delete->bind_param("ii", $this->DisciplineID, $this->AutoNumberingSchemeID);
   ...
   public function count() {
        ...
        global $connection;
        $returnvalue = false;
        $sql = 'SELECT count(*)  FROM autonumsch_dsp';
   ...
   public function loadArrayKeyValueSearch($searchTermArray) {
       ...
       $sql = "SELECT DisciplineID, AutoNumberingSchemeID FROM autonumsch_dsp $wherebit";
       if ($wherebit=='') { 
...
// TODO: *************** link to related tables 
  ...
  public function loadLinkedTo() { 
   ...
   public function keySelectAllConcatJSON($orderby='ASC') {
       ...
       $sql = "SELECT DisciplineID, AutoNumberingSchemeID, concat() FROM autonumsch_dsp order by DisciplineID, AutoNumberingSchemeID $order ";
       $comma = '';
       if ($result = $connection->query($sql)) { 
   ...
   public function keySelectDistinctJSON($field,$orderby='ASC') {
       ...
       if ($this->hasField($field)) { 
          ...
          if ($orderby=='ASC') { $order = 'ASC'; } else { $order = 'DESC'; } 
          $preparemysql = "SELECT DISTINCT $fieldesc FROM autonumsch_dsp order by $fieldesc $order ";
          $comma = '';
...
// can use Druid to regenerate the autonumsch_dsp.php file to reflect changes
// in the underlying database without overwriting your custom views of the data.
// 
class huh_autonumsch_dspView implements viewer
{
   var $model = null;
   ...
   public function getDetailsView($includeRelated=true, $editLinkURL='') {
       ...
       if ($editLinkURL!='') { 
          ...
          if (!$nullpk) { $returnvalue .= "<li>huh_autonumsch_dsp <a href='$editLinkURL'>Edit</a></li>\n";  } 
       }
       $returnvalue .= "<li>".huh_autonumsch_dsp::DISCIPLINEID.": ".$model->getDisciplineID()."</li>\n";
       $returnvalue .= "<li>".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID.": ".$model->getAutoNumberingSchemeID()."</li>\n";
       if ($includeRelated) { 
   ...
   public function getJSON() {
       $returnvalue = '{ ';
       $model = $this->model;
       $returnvalue .= '"'.huh_autonumsch_dsp::DISCIPLINEID.': "'.$model->getDisciplineID().'",';
       $returnvalue .= '"'.huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID.': "'.$model->getAutoNumberingSchemeID().'" }';
       $returnvalue .= '</ul>';
       return  $returnvalue;
   }
   ...
   public function getHeaderRow() {
       $returnvalue = '<tr>';
       $returnvalue .= "<th>".huh_autonumsch_dsp::DISCIPLINEID."</th>\n";
       $returnvalue .= "<th>".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID."</th>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
  ...
  function save'.$id.'() {
     ...
     dojo.xhrGet(saveprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_autonumsch_dsp">';
       $returnvalue .= "<div id='div_$formnane' >";
       $returnvalue .= "<div><label for=".huh_autonumsch_dsp::DISCIPLINEID.">DISCIPLINEID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_autonumsch_dsp::DISCIPLINEID." id=".huh_autonumsch_dsp::DISCIPLINEID." value='".$model->getDisciplineID()."'  style=' width:".huh_autonumsch_dsp::DISCIPLINEID_SIZE ."em;  '  maxlength='".huh_autonumsch_dsp::DISCIPLINEID_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID.">AUTONUMBERINGSCHEMEID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID." id=".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID." value='".$model->getAutoNumberingSchemeID()."'  style=' width:".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID_SIZE ."em;  '  maxlength='".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID_SIZE ."' ></div>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
  ...
  function deleterecord'.$id.'() {
     ...
     dojo.xhrGet(deleteprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_table value="huh_autonumsch_dsp">';
       $returnvalue .= '<input type=hidden name=DisciplineID, AutoNumberingSchemeID id=druid_table value="'.$id.'">';
       $returnvalue .= '<ul>';
   ...
   public function getEditFormView($includeRelated=true) {
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_autonumsch_dsp">';
       $returnvalue .= '<ul>';
       $returnvalue .= "<li>DISCIPLINEID<input type=text name=".huh_autonumsch_dsp::DISCIPLINEID." id=".huh_autonumsch_dsp::DISCIPLINEID." value='".$model->getDisciplineID()."'  size='".huh_autonumsch_dsp::DISCIPLINEID_SIZE ."'  maxlength='".huh_autonumsch_dsp::DISCIPLINEID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>AUTONUMBERINGSCHEMEID<input type=text name=".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID." id=".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID." value='".$model->getAutoNumberingSchemeID()."'  size='".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID_SIZE ."'  maxlength='".huh_autonumsch_dsp::AUTONUMBERINGSCHEMEID_SIZE ."' ></li>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
