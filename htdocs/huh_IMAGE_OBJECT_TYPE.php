...
/*  Customize these to limit each variable to narrowest possible set of known good values. 
  ...
  $NAME = substr(preg_replace('/[^A-Za-z0-9\.\.\ \[NULL\]]/','',$_GET['NAME']), 0, 100);
...
*/
   ...
   // constructor 
   function huh_IMAGE_OBJECT_TYPE(){
       $this->ID = NULL;
       $this->NAME = '';
       $this->DESCRIPTION = '';
       $this->dirty = false;
...
/*ID*/
   ...
   public function setID($ID) {
       if (strlen($ID) > huh_IMAGE_OBJECT_TYPE::ID_SIZE) { 
...
/*NAME*/
   ...
   public function setNAME($NAME) {
       if (strlen($NAME) > huh_IMAGE_OBJECT_TYPE::NAME_SIZE) { 
           throw new Exception('Value exceeds field length.');
       } 
...
/*DESCRIPTION*/
   ...
   public function setDESCRIPTION($DESCRIPTION) {
       if (strlen($DESCRIPTION) > huh_IMAGE_OBJECT_TYPE::DESCRIPTION_SIZE) { 
           throw new Exception('Value exceeds field length.');
       } 
  ...
  const C_DESCRIPTIONMYSQLI_TYPE = 's';
   ...
   public function load($pk) {
        ...
        if($this->ID != NULL) {
           $sql = 'SELECT ID, NAME, DESCRIPTION FROM IMAGE_OBJECT_TYPE WHERE ID = '.$this->ID ;

           $preparesql = 'SELECT ID, NAME, DESCRIPTION FROM IMAGE_OBJECT_TYPE WHERE ID = ? ';
   ...
   public function save() {
        ...
        if ($this->ID!= NULL) {
            $sql  = 'UPDATE  IMAGE_OBJECT_TYPE SET ';
            $isInsert = false;
            $sql .=  "NAME = ? ";
            $sql .=  ", DESCRIPTION = ? ";
            ...
            $sql  = 'INSERT INTO IMAGE_OBJECT_TYPE ';
            $isInsert = true;
if ($this->PK==NULL) { throw new Exception('Can\'t insert record with null primary key for this table'); }
   ...
   public function delete() {
        ...
        if($this->ID != NULL) {
           ...
           $preparedsql = 'SELECT  FROM IMAGE_OBJECT_TYPE WHERE  and ID = ?  ' ;
        if ($statement = $connection->prepare($preparedsql)) { 
           $statement->bind_param("iss", $this->ID, $this->NAME, $this->DESCRIPTION);
           $statement->execute();
           $statement->store_result();
           if ($statement->num_rows()==1) {
                $sql = 'DELETE FROM IMAGE_OBJECT_TYPE WHERE  and ID = ?  ';
   ...
   public function count() {
        ...
        global $connection;
        $returnvalue = false;
        $sql = 'SELECT count(*)  FROM IMAGE_OBJECT_TYPE';
   ...
   public function loadArrayKeyValueSearch($searchTermArray) {
       ...
       $sql = "SELECT ID FROM IMAGE_OBJECT_TYPE $wherebit";
       if ($wherebit=='') { 
...
// TODO: *************** link to related tables 
   ...
   public function keySelectAllConcatJSON($orderby='ASC') {
       ...
       $sql = "SELECT ID, concat(IFNULL(NAME,'') || ' ' || IFNULL(DESCRIPTION,'')) FROM IMAGE_OBJECT_TYPE order by ID $order ";
       $comma = '';
       if ($result = $connection->query($sql)) { 
   ...
   public function keySelectDistinctJSON($field,$orderby='ASC') {
       ...
       if ($this->hasField($field)) { 
          ...
          if ($orderby=='ASC') { $order = 'ASC'; } else { $order = 'DESC'; } 
          $preparemysql = "SELECT DISTINCT $fieldesc FROM IMAGE_OBJECT_TYPE order by $fieldesc $order ";
          $comma = '';
...
// can use Druid to regenerate the IMAGE_OBJECT_TYPE.php file to reflect changes
// in the underlying database without overwriting your custom views of the data.
// 
class huh_IMAGE_OBJECT_TYPEView implements viewer
{
   var $model = null;
   ...
   public function getDetailsView($includeRelated=true, $editLinkURL='') {
       ...
       if ($editLinkURL!='') { 
          ...
          if (!$nullpk) { $returnvalue .= "<li>huh_IMAGE_OBJECT_TYPE <a href='$editLinkURL'>Edit</a></li>\n";  } 
       }
       $returnvalue .= "<li>".huh_IMAGE_OBJECT_TYPE::ID.": ".$model->getID()."</li>\n";
       $returnvalue .= "<li>".huh_IMAGE_OBJECT_TYPE::NAME.": ".$model->getNAME()."</li>\n";
       $returnvalue .= "<li>".huh_IMAGE_OBJECT_TYPE::DESCRIPTION.": ".$model->getDESCRIPTION()."</li>\n";
       if ($includeRelated) { 
   ...
   public function getJSON() {
       $returnvalue = '{ ';
       $model = $this->model;
       $returnvalue .= '"'.huh_IMAGE_OBJECT_TYPE::ID.': "'.$model->getID().'",';
       $returnvalue .= '"'.huh_IMAGE_OBJECT_TYPE::NAME.': "'.$model->getNAME().'",';
       $returnvalue .= '"'.huh_IMAGE_OBJECT_TYPE::DESCRIPTION.': "'.$model->getDESCRIPTION().'" }';
       $returnvalue .= '</ul>';
       return  $returnvalue;
   }
   ...
   public function getHeaderRow() {
       $returnvalue = '<tr>';
       $returnvalue .= "<th>".huh_IMAGE_OBJECT_TYPE::ID."</th>\n";
       $returnvalue .= "<th>".huh_IMAGE_OBJECT_TYPE::NAME."</th>\n";
       $returnvalue .= "<th>".huh_IMAGE_OBJECT_TYPE::DESCRIPTION."</th>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
  ...
  function save'.$id.'() {
     ...
     dojo.xhrGet(saveprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_IMAGE_OBJECT_TYPE">';
       $returnvalue .= "<div id='div_$formnane' >";
       if($addform) { 
          $returnvalue .= "Add a new ID"; 
       } else { 
         $returnvalue .= "<div><label for=".huh_IMAGE_OBJECT_TYPE::ID.">ID</label><input  dojoType='dijit.form.ValidationTextBox'  name=".huh_IMAGE_OBJECT_TYPE::ID." id=".huh_IMAGE_OBJECT_TYPE::ID." value='".$model->getID()."'  style=' width:".huh_IMAGE_OBJECT_TYPE::ID_SIZE ."em;  '  maxlength='".huh_IMAGE_OBJECT_TYPE::ID_SIZE ."' ></div>\n";
       }  
       $returnvalue .= "<div><label for=".huh_IMAGE_OBJECT_TYPE::NAME.">NAME</label><textarea  dojoType='dijit.form.Textarea'  style=' width:51em; border:1px solid grey; '  name=".huh_IMAGE_OBJECT_TYPE::NAME." id=".huh_IMAGE_OBJECT_TYPE::NAME." >".$model->getNAME()."</textarea></div>\n";
       $returnvalue .= "<div><label for=".huh_IMAGE_OBJECT_TYPE::DESCRIPTION.">DESCRIPTION</label><textarea  dojoType='dijit.form.Textarea'  style=' width:51em; border:1px solid grey; '  name=".huh_IMAGE_OBJECT_TYPE::DESCRIPTION." id=".huh_IMAGE_OBJECT_TYPE::DESCRIPTION." >".$model->getDESCRIPTION()."</textarea></div>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
  ...
  function deleterecord'.$id.'() {
     ...
     dojo.xhrGet(deleteprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_table value="huh_IMAGE_OBJECT_TYPE">';
   ...
   public function getEditFormView($includeRelated=true) {
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_IMAGE_OBJECT_TYPE">';
       $returnvalue .= '<ul>';
       $returnvalue .= "<li>ID<input type=text name=".huh_IMAGE_OBJECT_TYPE::ID." id=".huh_IMAGE_OBJECT_TYPE::ID." value='".$model->getID()."'  size='".huh_IMAGE_OBJECT_TYPE::ID_SIZE ."'  maxlength='".huh_IMAGE_OBJECT_TYPE::ID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>NAME<input type=text name=".huh_IMAGE_OBJECT_TYPE::NAME." id=".huh_IMAGE_OBJECT_TYPE::NAME." value='".$model->getNAME()."'  size='51'  maxlength='".huh_IMAGE_OBJECT_TYPE::NAME_SIZE ."' ></li>\n";
       $returnvalue .= "<li>DESCRIPTION<input type=text name=".huh_IMAGE_OBJECT_TYPE::DESCRIPTION." id=".huh_IMAGE_OBJECT_TYPE::DESCRIPTION." value='".$model->getDESCRIPTION()."'  size='51'  maxlength='".huh_IMAGE_OBJECT_TYPE::DESCRIPTION_SIZE ."' ></li>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
