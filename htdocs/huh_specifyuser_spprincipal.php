<?php
//==============================================================================
//===   specifyuser_spprincipal.php                         
//===   Autogenerated by Druid from MySQL db Build:6
//==============================================================================

include_once("druid_interfaces.php");

class huh_specifyuser_spprincipal implements  model , loadableModel, saveableModel, tableSchema 
{
...
*/
   ...
   private $loaded;
   private $error;
   const FIELDLIST = ' SpecifyUserID, SpPrincipalID, ';
   const PKFIELDLIST = ' SpecifyUserID, SpPrincipalID, ';
   const NUMBER_OF_PRIMARY_KEYS = 2;
   private $primaryKeyArray = array( 1 => 'SpecifyUserID' , 2 => 'SpPrincipalID'  ) ;
   private $allFieldsArray = array( 0 => 'SpecifyUserID' , 1 => 'SpPrincipalID'  ) ;
   private $selectDistinctFieldsArray = array(  ) ;

   //---------------------------------------------------------------------------

   // constructor 
   function huh_specifyuser_spprincipal(){
...
/*SpecifyUserID*/
   ...
   public function setSpecifyUserID($SpecifyUserID) {
       if (strlen(preg_replace('/[^0-9]/','',$SpecifyUserID)) > huh_specifyuser_spprincipal::SPECIFYUSERID_SIZE) { 
...
/*SpPrincipalID*/
   ...
   public function setSpPrincipalID($SpPrincipalID) {
       if (strlen(preg_replace('/[^0-9]/','',$SpPrincipalID)) > huh_specifyuser_spprincipal::SPPRINCIPALID_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
   ...
   public function PK() { // get value of primary keys as comma separated list of values 
        $returnvalue = '';
        $comma = '';
        $returnvalue .= $comma . $this->getSpecifyUserID();
        $comma = ', ';
  ...
  const C_SpPrincipalIDMYSQLI_TYPE = 'i';
   ...
   public function load($pk) {
        ...
        if(SpecifyUserID != NULL  && SpPrincipalID != NULL ) {
           $sql = 'SELECT SpecifyUserID, SpPrincipalID FROM specifyuser_spprincipal WHERE SpecifyUserID = "'. $this->SpecifyUserID.'"  and SpPrincipalID = "'. $this->SpPrincipalID.'" ' ;

           $preparesql = 'SELECT SpecifyUserID, SpPrincipalID FROM specifyuser_spprincipal WHERE SpecifyUserID = ? SpPrincipalID = ?  ';
           $values = array('pk0' => $this->SpecifyUserID);           $values = array('pk1' => $this->SpPrincipalID);
           if ($statement = $connection->prepare($preparesql)) { 
              $statement->bind_param("ii", $this->SpecifyUserID, $this->SpPrincipalID);
              $statement->execute();
              $statement->bind_result($this->SpecifyUserID, $this->SpPrincipalID);
              $statement->fetch();
              $statement->close();
           }
   ...
   public function save($doinsert=false) {
        ...
        global $connection;
        $returnvalue = false;
        if ($doinsert) {            $sql  = 'UPDATE  specifyuser_spprincipal SET ';
            $isInsert = false;
            $sql .=  "SpecifyUserID = ? ";
            $sql .=  "SpPrincipalID = ? ";
            ...
            $sql  = 'INSERT INTO specifyuser_spprincipal ';
            $isInsert = true;
            $sql .= '( SpecifyUserID ,  SpPrincipalID ) VALUES (';
            $sql .=  "  ? ";
            $sql .=  " ,  ? ";
            $sql .= ')';
        ...
        if ($statement = $connection->prepare($sql)) { 
           if ($this->SpecifyUserID!= NULL & $this->SpPrincipalID!= NULL ) {
              $statement->bind_param("iiii", $this->SpecifyUserID , $this->SpPrincipalID , $this->SpecifyUserID , $this->SpPrincipalID );
           } else { 
              $statement->bind_param("ii", $this->SpecifyUserID , $this->SpPrincipalID );
           } 
           $statement->execute();
   ...
   public function delete() {
        ...
        if(SpecifyUserID != NULL  && SpPrincipalID != NULL ) {
           ...
           $preparedsql = 'SELECT  FROM specifyuser_spprincipal WHERE  and SpecifyUserID = ?  and SpPrincipalID = ?  ' ;
        if ($statement = $connection->prepare($preparedsql)) { 
           $statement->bind_param("ii", $this->SpecifyUserID, $this->SpPrincipalID);
           $statement->execute();
           $statement->store_result();
           if ($statement->num_rows()==1) {
                $sql = 'DELETE FROM specifyuser_spprincipal WHERE  and SpecifyUserID = ?  and SpPrincipalID = ?  ';
                if ($stmt_delete = $connection->prepare($sql)) { 
                   $stmt_delete->bind_param("ii", $this->SpecifyUserID, $this->SpPrincipalID);
   ...
   public function count() {
        ...
        global $connection;
        $returnvalue = false;
        $sql = 'SELECT count(*)  FROM specifyuser_spprincipal';
   ...
   public function loadArrayKeyValueSearch($searchTermArray) {
       ...
       $sql = "SELECT SpecifyUserID, SpPrincipalID FROM specifyuser_spprincipal $wherebit";
       if ($wherebit=='') { 
          ...
          while ($statement->fetch()) {
              $obj = new huh_specifyuser_spprincipal();
...
// TODO: *************** link to related tables 
  ...
  public function loadLinkedTo() { 
   ...
   public function keySelectAllConcatJSON($orderby='ASC') {
       ...
       $sql = "SELECT SpecifyUserID, SpPrincipalID, concat() FROM specifyuser_spprincipal order by SpecifyUserID, SpPrincipalID $order ";
       $comma = '';
       if ($result = $connection->query($sql)) { 
   ...
   public function keySelectDistinctJSON($field,$orderby='ASC') {
       ...
       if ($this->hasField($field)) { 
          ...
          if ($orderby=='ASC') { $order = 'ASC'; } else { $order = 'DESC'; } 
          $preparemysql = "SELECT DISTINCT $fieldesc FROM specifyuser_spprincipal order by $fieldesc $order ";
          $comma = '';
...
// can use Druid to regenerate the specifyuser_spprincipal.php file to reflect changes
// in the underlying database without overwriting your custom views of the data.
// 
class huh_specifyuser_spprincipalView implements viewer
{
   var $model = null;
   ...
   public function getDetailsView($includeRelated=true, $editLinkURL='') {
       ...
       if ($editLinkURL!='') { 
          ...
          if (!$nullpk) { $returnvalue .= "<li>huh_specifyuser_spprincipal <a href='$editLinkURL'>Edit</a></li>\n";  } 
       }
       $returnvalue .= "<li>".huh_specifyuser_spprincipal::SPECIFYUSERID.": ".$model->getSpecifyUserID()."</li>\n";
       $returnvalue .= "<li>".huh_specifyuser_spprincipal::SPPRINCIPALID.": ".$model->getSpPrincipalID()."</li>\n";
       if ($includeRelated) { 
   ...
   public function getJSON() {
       $returnvalue = '{ ';
       $model = $this->model;
       $returnvalue .= '"'.huh_specifyuser_spprincipal::SPECIFYUSERID.': "'.$model->getSpecifyUserID().'",';
       $returnvalue .= '"'.huh_specifyuser_spprincipal::SPPRINCIPALID.': "'.$model->getSpPrincipalID().'" }';
       $returnvalue .= '</ul>';
       return  $returnvalue;
   }
   ...
   public function getHeaderRow() {
       $returnvalue = '<tr>';
       $returnvalue .= "<th>".huh_specifyuser_spprincipal::SPECIFYUSERID."</th>\n";
       $returnvalue .= "<th>".huh_specifyuser_spprincipal::SPPRINCIPALID."</th>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
  ...
  function save'.$id.'() {
     ...
     dojo.xhrGet(saveprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_specifyuser_spprincipal">';
       $returnvalue .= "<div id='div_$formnane' >";
       $returnvalue .= "<div><label for=".huh_specifyuser_spprincipal::SPECIFYUSERID.">SPECIFYUSERID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_specifyuser_spprincipal::SPECIFYUSERID." id=".huh_specifyuser_spprincipal::SPECIFYUSERID." value='".$model->getSpecifyUserID()."'  style=' width:".huh_specifyuser_spprincipal::SPECIFYUSERID_SIZE ."em;  '  maxlength='".huh_specifyuser_spprincipal::SPECIFYUSERID_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_specifyuser_spprincipal::SPPRINCIPALID.">SPPRINCIPALID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_specifyuser_spprincipal::SPPRINCIPALID." id=".huh_specifyuser_spprincipal::SPPRINCIPALID." value='".$model->getSpPrincipalID()."'  style=' width:".huh_specifyuser_spprincipal::SPPRINCIPALID_SIZE ."em;  '  maxlength='".huh_specifyuser_spprincipal::SPPRINCIPALID_SIZE ."' ></div>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
  ...
  function deleterecord'.$id.'() {
     ...
     dojo.xhrGet(deleteprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_table value="huh_specifyuser_spprincipal">';
       $returnvalue .= '<input type=hidden name=SpecifyUserID, SpPrincipalID id=druid_table value="'.$id.'">';
       $returnvalue .= '<ul>';
   ...
   public function getEditFormView($includeRelated=true) {
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_specifyuser_spprincipal">';
       $returnvalue .= '<ul>';
       $returnvalue .= "<li>SPECIFYUSERID<input type=text name=".huh_specifyuser_spprincipal::SPECIFYUSERID." id=".huh_specifyuser_spprincipal::SPECIFYUSERID." value='".$model->getSpecifyUserID()."'  size='".huh_specifyuser_spprincipal::SPECIFYUSERID_SIZE ."'  maxlength='".huh_specifyuser_spprincipal::SPECIFYUSERID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>SPPRINCIPALID<input type=text name=".huh_specifyuser_spprincipal::SPPRINCIPALID." id=".huh_specifyuser_spprincipal::SPPRINCIPALID." value='".$model->getSpPrincipalID()."'  size='".huh_specifyuser_spprincipal::SPPRINCIPALID_SIZE ."'  maxlength='".huh_specifyuser_spprincipal::SPPRINCIPALID_SIZE ."' ></li>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
