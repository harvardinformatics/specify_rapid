...
class huh_datatype implements  model , loadableModel, saveableModel, tableSchema 
{
...
/*  Customize these to limit each variable to narrowest possible set of known good values. 
  ...
  $Name = substr(preg_replace('/[^A-Za-z0-9\.\.\ \[NULL\]]/','',$_GET['Name']), 0, 20);
...
*/
   ...
   private $loaded;
   private $error;
   const FIELDLIST = ' DataTypeID, TimestampCreated, TimestampModified, Version, Name, ModifiedByAgentID, CreatedByAgentID, ';
   const PKFIELDLIST = ' DataTypeID, ';
   const NUMBER_OF_PRIMARY_KEYS = 1;
   private $primaryKeyArray = array( 1 => 'DataTypeID'  ) ;
   private $allFieldsArray = array( 0 => 'DataTypeID' , 1 => 'TimestampCreated' , 2 => 'TimestampModified' , 3 => 'Version' , 4 => 'Name' , 5 => 'ModifiedByAgentID' , 6 => 'CreatedByAgentID'  ) ;
   private $selectDistinctFieldsArray = array( 5 => 'ModifiedByAgentID' , 6 => 'CreatedByAgentID'  ) ;

   //---------------------------------------------------------------------------

   // constructor 
   function huh_datatype(){
...
/*DataTypeID*/
   ...
   public function setDataTypeID($DataTypeID) {
       if (strlen(preg_replace('/[^0-9]/','',$DataTypeID)) > huh_datatype::DATATYPEID_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
       $DataTypeID = trim($DataTypeID);
       if (!ctype_digit(strval($DataTypeID)) && trim(strval($DataTypeID))!='' ) {
...
/*TimestampCreated*/
   ...
   public function setTimestampCreated($TimestampCreated) {
       if (strlen($TimestampCreated) > huh_datatype::TIMESTAMPCREATED_SIZE) { 
...
/*TimestampModified*/
   ...
   public function setTimestampModified($TimestampModified) {
       if (strlen($TimestampModified) > huh_datatype::TIMESTAMPMODIFIED_SIZE) { 
           throw new Exception('Value exceeds field length.');
       } 
...
/*Version*/
   ...
   public function setVersion($Version) {
       if (strlen(preg_replace('/[^0-9]/','',$Version)) > huh_datatype::VERSION_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
...
/*Name*/
   ...
   public function setName($Name) {
       if (strlen($Name) > huh_datatype::NAME_SIZE) { 
           throw new Exception('Value exceeds field length.');
       } 
...
/*ModifiedByAgentID*/
   ...
   public function setModifiedByAgentID($ModifiedByAgentID) {
       if (strlen(preg_replace('/[^0-9]/','',$ModifiedByAgentID)) > huh_datatype::MODIFIEDBYAGENTID_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
...
/*CreatedByAgentID*/
   ...
   public function setCreatedByAgentID($CreatedByAgentID) {
       if (strlen(preg_replace('/[^0-9]/','',$CreatedByAgentID)) > huh_datatype::CREATEDBYAGENTID_SIZE) { 
           throw new Exception('Value has too many digits for the field length.');
       } 
   ...
   public function PK() { // get value of primary key 
        $returnvalue = '';
        $returnvalue .= $this->getDataTypeID();
        return $returnvalue;
   }
   public function PKArray() { // get name and value of primary key fields 
        $returnvalue = array();
        $returnvalue['DataTypeID'] = $this->getDataTypeID();
        return $returnvalue;
   }
  ...
  const C_CreatedByAgentIDMYSQLI_TYPE = 'i';
   ...
   public function load($pk) {
        ...
        try {
             if (is_array($pk)) { 
                 $this->setDataTypeID($pk[DataTypeID]);
             } else { ;
                 $this->setDataTypeID($pk);
             };
        } 
        catch (Exception $e) { 
        ...
        if($this->DataTypeID != NULL) {
           $sql = 'SELECT DataTypeID, TimestampCreated, TimestampModified, Version, Name, ModifiedByAgentID, CreatedByAgentID FROM datatype WHERE DataTypeID = '.$this->DataTypeID ;

           $preparesql = 'SELECT DataTypeID, TimestampCreated, TimestampModified, Version, Name, ModifiedByAgentID, CreatedByAgentID FROM datatype WHERE DataTypeID = ? ';

           if ($statement = $connection->prepare($preparesql)) { 
              $statement->bind_param("i", $this->DataTypeID);
              $statement->execute();
              $statement->bind_result($this->DataTypeID, $this->TimestampCreated, $this->TimestampModified, $this->Version, $this->Name, $this->ModifiedByAgentID, $this->CreatedByAgentID);
              $statement->fetch();
              $statement->close();
           }
   ...
   public function save() {
        ...
        if ($this->DataTypeID!= NULL) {
            ...
            $sql .= "  WHERE DataTypeID = ? ";
        } else {
            $sql  = 'INSERT INTO datatype ';
            $isInsert = true;
if ($this->PK==NULL) { throw new Exception('Can\'t insert record with null primary key for this table'); }
            $sql .= '( DataTypeID ,  TimestampCreated ,  TimestampModified ,  Version ,  Name ,  ModifiedByAgentID ,  CreatedByAgentID ) VALUES (';
            $sql .=  "  ? ";
            $sql .=  " ,  ? ";
            $sql .=  " ,  ? ";
            $sql .=  " ,  ? ";
        ...
        if ($statement = $connection->prepare($sql)) { 
           if ($this->DataTypeID!= NULL ) {
              $statement->bind_param("issisiii", $this->DataTypeID , $this->TimestampCreated , $this->TimestampModified , $this->Version , $this->Name , $this->ModifiedByAgentID , $this->CreatedByAgentID , $this->DataTypeID );
           } else { 
              $statement->bind_param("issisii", $this->DataTypeID , $this->TimestampCreated , $this->TimestampModified , $this->Version , $this->Name , $this->ModifiedByAgentID , $this->CreatedByAgentID );
           } 
           $statement->execute();
   ...
   public function delete() {
        ...
        if($this->DataTypeID != NULL) {
           ...
           $preparedsql = 'SELECT  FROM datatype WHERE  and DataTypeID = ?  ' ;
        if ($statement = $connection->prepare($preparedsql)) { 
           $statement->bind_param("issisii", $this->DataTypeID, $this->TimestampCreated, $this->TimestampModified, $this->Version, $this->Name, $this->ModifiedByAgentID, $this->CreatedByAgentID);
           $statement->execute();
           $statement->store_result();
           if ($statement->num_rows()==1) {
                $sql = 'DELETE FROM datatype WHERE  and DataTypeID = ?  ';
                if ($stmt_delete = $connection->prepare($sql)) { 
                   $stmt_delete->bind_param("issisii", $this->DataTypeID, $this->TimestampCreated, $this->TimestampModified, $this->Version, $this->Name, $this->ModifiedByAgentID, $this->CreatedByAgentID);
   ...
   public function count() {
        ...
        global $connection;
        $returnvalue = false;
        $sql = 'SELECT count(*)  FROM datatype';
   ...
   public function loadArrayKeyValueSearch($searchTermArray) {
       ...
       $sql = "SELECT DataTypeID FROM datatype $wherebit";
       if ($wherebit=='') { 
...
// TODO: *************** link to related tables 
  ...
  public function loadLinkedTo() { 
   ...
   public function keySelectAllConcatJSON($orderby='ASC') {
       ...
       $sql = "SELECT DataTypeID, concat(IFNULL(TimestampCreated,'') || ' ' || IFNULL(TimestampModified,'') || ' ' || IFNULL(Version,'') || ' ' || IFNULL(Name,'') || ' ' || IFNULL(ModifiedByAgentID,'') || ' ' || IFNULL(CreatedByAgentID,'')) FROM datatype order by ModifiedByAgentID, CreatedByAgentID $order ";
       $comma = '';
       if ($result = $connection->query($sql)) { 
   ...
   public function keySelectDistinctJSON($field,$orderby='ASC') {
       ...
       if ($this->hasField($field)) { 
          ...
          if ($orderby=='ASC') { $order = 'ASC'; } else { $order = 'DESC'; } 
          $preparemysql = "SELECT DISTINCT $fieldesc FROM datatype order by $fieldesc $order ";
          $comma = '';
   ...
   public function loadArrayByModifiedByAgentID($searchTerm) {
        ...
        if (!(strpos($searchTerm,"%")===false)) { $operator = "like"; }
        if (!(strpos($searchTerm,"_")===false)) { $operator = "like"; }
        $sql = "SELECT DataTypeID FROM datatype WHERE ModifiedByAgentID $operator '$searchTerm'";
        $preparedsql = "SELECT DataTypeID FROM datatype WHERE ModifiedByAgentID $operator ? ";
        if ($statement = $connection->prepare($preparedsql)) { 
   ...
   public function loadArrayByCreatedByAgentID($searchTerm) {
        ...
        if (!(strpos($searchTerm,"%")===false)) { $operator = "like"; }
        if (!(strpos($searchTerm,"_")===false)) { $operator = "like"; }
        $sql = "SELECT DataTypeID FROM datatype WHERE CreatedByAgentID $operator '$searchTerm'";
        $preparedsql = "SELECT DataTypeID FROM datatype WHERE CreatedByAgentID $operator ? ";
        if ($statement = $connection->prepare($preparedsql)) { 
            $statement->bind_param("s", $searchTerm);
            $statement->execute();
            $statement->bind_result($id);
   ...
   public function selectDistinctModifiedByAgentID($startline,$link,$endline,$includecount=false,$orderbycount=false) {
        ...
        $sql = "SELECT count(*) as druid_ct, ModifiedByAgentID FROM datatype group by ModifiedByAgentID order by $order ";
        if ($result = $connection->query($sql)) { 
           while ($row = $result->fetch_row()) {
              if ($row) {
                  $count = $row[0];
   ...
   public function selectDistinctCreatedByAgentID($startline,$link,$endline,$includecount=false,$orderbycount=false) {
        ...
        if ($orderbycount) { $order = ' druid_ct DESC '; } 
        $sql = "SELECT count(*) as druid_ct, CreatedByAgentID FROM datatype group by CreatedByAgentID order by $order ";
        if ($result = $connection->query($sql)) { 
           while ($row = $result->fetch_row()) {
              if ($row) {
                  $count = $row[0];
...
// can use Druid to regenerate the datatype.php file to reflect changes
// in the underlying database without overwriting your custom views of the data.
// 
class huh_datatypeView implements viewer
{
   var $model = null;
   ...
   public function getDetailsView($includeRelated=true, $editLinkURL='') {
       ...
       if ($editLinkURL!='') { 
          ...
          if (!$nullpk) { $returnvalue .= "<li>huh_datatype <a href='$editLinkURL'>Edit</a></li>\n";  } 
       }
       $returnvalue .= "<li>".huh_datatype::DATATYPEID.": ".$model->getDataTypeID()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::TIMESTAMPCREATED.": ".$model->getTimestampCreated()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::TIMESTAMPMODIFIED.": ".$model->getTimestampModified()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::VERSION.": ".$model->getVersion()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::NAME.": ".$model->getName()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::MODIFIEDBYAGENTID.": ".$model->getModifiedByAgentID()."</li>\n";
       $returnvalue .= "<li>".huh_datatype::CREATEDBYAGENTID.": ".$model->getCreatedByAgentID()."</li>\n";
       if ($includeRelated) { 
   ...
   public function getJSON() {
       $returnvalue = '{ ';
       $model = $this->model;
       $returnvalue .= '"'.huh_datatype::DATATYPEID.': "'.$model->getDataTypeID().'",';
       $returnvalue .= '"'.huh_datatype::TIMESTAMPCREATED.': "'.$model->getTimestampCreated().'",';
       $returnvalue .= '"'.huh_datatype::TIMESTAMPMODIFIED.': "'.$model->getTimestampModified().'",';
       $returnvalue .= '"'.huh_datatype::VERSION.': "'.$model->getVersion().'",';
       $returnvalue .= '"'.huh_datatype::NAME.': "'.$model->getName().'",';
       $returnvalue .= '"'.huh_datatype::MODIFIEDBYAGENTID.': "'.$model->getModifiedByAgentID().'",';
       $returnvalue .= '"'.huh_datatype::CREATEDBYAGENTID.': "'.$model->getCreatedByAgentID().'" }';
       $returnvalue .= '</ul>';
       return  $returnvalue;
   }
   ...
   public function getHeaderRow() {
       $returnvalue = '<tr>';
       $returnvalue .= "<th>".huh_datatype::DATATYPEID."</th>\n";
       $returnvalue .= "<th>".huh_datatype::TIMESTAMPCREATED."</th>\n";
       $returnvalue .= "<th>".huh_datatype::TIMESTAMPMODIFIED."</th>\n";
       $returnvalue .= "<th>".huh_datatype::VERSION."</th>\n";
       $returnvalue .= "<th>".huh_datatype::NAME."</th>\n";
       $returnvalue .= "<th>".huh_datatype::MODIFIEDBYAGENTID."</th>\n";
       $returnvalue .= "<th>".huh_datatype::CREATEDBYAGENTID."</th>\n";
       $returnvalue .= '</tr>';
       return  $returnvalue;
   }
  ...
  function save'.$id.'() {
     ...
     dojo.xhrGet(saveprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_datatype">';
       $returnvalue .= "<div id='div_$formnane' >";
       if($addform) { 
          $returnvalue .= "Add a new DataTypeID"; 
       } else { 
         $returnvalue .= "<div><label for=".huh_datatype::DATATYPEID.">DATATYPEID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_datatype::DATATYPEID." id=".huh_datatype::DATATYPEID." value='".$model->getDataTypeID()."'  style=' width:".huh_datatype::DATATYPEID_SIZE ."em;  '  maxlength='".huh_datatype::DATATYPEID_SIZE ."' ></div>\n";
       }  
       $returnvalue .= "<div><label for=".huh_datatype::TIMESTAMPCREATED.">TIMESTAMPCREATED</label><input  dojoType='dijit.form.ValidationTextBox'  name=".huh_datatype::TIMESTAMPCREATED." id=".huh_datatype::TIMESTAMPCREATED." value='".$model->getTimestampCreated()."'  style=' width:".huh_datatype::TIMESTAMPCREATED_SIZE ."em;  '  maxlength='".huh_datatype::TIMESTAMPCREATED_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_datatype::TIMESTAMPMODIFIED.">TIMESTAMPMODIFIED</label><input  dojoType='dijit.form.ValidationTextBox'  name=".huh_datatype::TIMESTAMPMODIFIED." id=".huh_datatype::TIMESTAMPMODIFIED." value='".$model->getTimestampModified()."'  style=' width:".huh_datatype::TIMESTAMPMODIFIED_SIZE ."em;  '  maxlength='".huh_datatype::TIMESTAMPMODIFIED_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_datatype::VERSION.">VERSION</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_datatype::VERSION." id=".huh_datatype::VERSION." value='".$model->getVersion()."'  style=' width:".huh_datatype::VERSION_SIZE ."em;  '  maxlength='".huh_datatype::VERSION_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_datatype::NAME.">NAME</label><input  dojoType='dijit.form.ValidationTextBox'  name=".huh_datatype::NAME." id=".huh_datatype::NAME." value='".$model->getName()."'  style=' width:".huh_datatype::NAME_SIZE ."em;  '  maxlength='".huh_datatype::NAME_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_datatype::MODIFIEDBYAGENTID.">MODIFIEDBYAGENTID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_datatype::MODIFIEDBYAGENTID." id=".huh_datatype::MODIFIEDBYAGENTID." value='".$model->getModifiedByAgentID()."'  style=' width:".huh_datatype::MODIFIEDBYAGENTID_SIZE ."em;  '  maxlength='".huh_datatype::MODIFIEDBYAGENTID_SIZE ."' ></div>\n";
       $returnvalue .= "<div><label for=".huh_datatype::CREATEDBYAGENTID.">CREATEDBYAGENTID</label><input  dojoType='dijit.form.ValidationTextBox'  regExp='^[0-9-]*'  invalidMessage='The value entered is not valid.  It must be an integer.' name=".huh_datatype::CREATEDBYAGENTID." id=".huh_datatype::CREATEDBYAGENTID." value='".$model->getCreatedByAgentID()."'  style=' width:".huh_datatype::CREATEDBYAGENTID_SIZE ."em;  '  maxlength='".huh_datatype::CREATEDBYAGENTID_SIZE ."' ></div>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
  ...
  function deleterecord'.$id.'() {
     ...
     dojo.xhrGet(deleteprocessor'.$id.');
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_table value="huh_datatype">';
       $returnvalue .= '<input type=hidden name=DataTypeID id=druid_table value="'.$id.'">';
       $returnvalue .= '<ul>';
   ...
   public function getEditFormView($includeRelated=true) {
       ...
       $returnvalue .= '<input type=hidden name=druid_table id=druid_action value="huh_datatype">';
       $returnvalue .= '<ul>';
       $returnvalue .= "<li>DATATYPEID<input type=text name=".huh_datatype::DATATYPEID." id=".huh_datatype::DATATYPEID." value='".$model->getDataTypeID()."'  size='".huh_datatype::DATATYPEID_SIZE ."'  maxlength='".huh_datatype::DATATYPEID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>TIMESTAMPCREATED<input type=text name=".huh_datatype::TIMESTAMPCREATED." id=".huh_datatype::TIMESTAMPCREATED." value='".$model->getTimestampCreated()."'  size='".huh_datatype::TIMESTAMPCREATED_SIZE ."'  maxlength='".huh_datatype::TIMESTAMPCREATED_SIZE ."' ></li>\n";
       $returnvalue .= "<li>TIMESTAMPMODIFIED<input type=text name=".huh_datatype::TIMESTAMPMODIFIED." id=".huh_datatype::TIMESTAMPMODIFIED." value='".$model->getTimestampModified()."'  size='".huh_datatype::TIMESTAMPMODIFIED_SIZE ."'  maxlength='".huh_datatype::TIMESTAMPMODIFIED_SIZE ."' ></li>\n";
       $returnvalue .= "<li>VERSION<input type=text name=".huh_datatype::VERSION." id=".huh_datatype::VERSION." value='".$model->getVersion()."'  size='".huh_datatype::VERSION_SIZE ."'  maxlength='".huh_datatype::VERSION_SIZE ."' ></li>\n";
       $returnvalue .= "<li>NAME<input type=text name=".huh_datatype::NAME." id=".huh_datatype::NAME." value='".$model->getName()."'  size='".huh_datatype::NAME_SIZE ."'  maxlength='".huh_datatype::NAME_SIZE ."' ></li>\n";
       $returnvalue .= "<li>MODIFIEDBYAGENTID<input type=text name=".huh_datatype::MODIFIEDBYAGENTID." id=".huh_datatype::MODIFIEDBYAGENTID." value='".$model->getModifiedByAgentID()."'  size='".huh_datatype::MODIFIEDBYAGENTID_SIZE ."'  maxlength='".huh_datatype::MODIFIEDBYAGENTID_SIZE ."' ></li>\n";
       $returnvalue .= "<li>CREATEDBYAGENTID<input type=text name=".huh_datatype::CREATEDBYAGENTID." id=".huh_datatype::CREATEDBYAGENTID." value='".$model->getCreatedByAgentID()."'  size='".huh_datatype::CREATEDBYAGENTID_SIZE ."'  maxlength='".huh_datatype::CREATEDBYAGENTID_SIZE ."' ></li>\n";
       if ($includeRelated) { 
           // note that $includeRelated is provided as false in calls out to
           // related tables to prevent infinite loops between related objects.
        }
