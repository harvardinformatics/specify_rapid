...
class huh_taxon_CUSTOM extends huh_taxon {
   ...
   public function keySelectTaxonTaxonIDJSON($term) {
      ...
      //$returnvalue = '[';
      $returnvalue = '';
      $preparemysql = " SELECT t.taxonid, concat(t.fullname, if(t.author is null,'',concat(' ', t.author))), concat(t.fullname,if(t.author is null,'',concat(' ', t.author)), ' [',ifnull(t.groupnumber,''), if(t.rankid > 140, concat(', ', ifnull(tp.fullname,'')), ''),']',if(t.remarks is null,'',concat('[',t.remarks,']'))) as label FROM taxon t left join taxon tp on t.nodenumber < tp.HighestChildNodeNumber and t.NodeNumber > tp.NodeNumber and tp.rankid = 140 where t.fullname like ? order by t.fullname ASC, t.author ASC ";
      $comma = '';
      if ($stmt = $connection->prepare($preparemysql)) {
