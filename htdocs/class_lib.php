...
class huh_taxon_CUSTOM extends huh_taxon {
   ...
   public function keySelectTaxonTaxonIDJSON($term) {
      ...
      //$returnvalue = '[';
      $returnvalue = '';
      $preparemysql = " SELECT t.taxonid, concat(t.fullname, if(t.author is null,'',concat(' ', t.author))), concat(ifnull(concat(t.text1,': '),''),t.fullname,if(t.author is null,'',concat(' ', t.author)), ' [',ifnull(t.groupnumber,''), if(t.rankid > 140, concat(', ', ifnull(tp.fullname,'')), ''),']',if(t.remarks is null,'',concat('[',t.remarks,']'))) as label FROM taxon t left join taxon tp on t.nodenumber < tp.HighestChildNodeNumber and t.NodeNumber > tp.NodeNumber and tp.rankid = 140 where t.fullname like ? order by t.fullname ASC, t.author ASC ";
      $comma = '';
      if ($stmt = $connection->prepare($preparemysql)) {
...
class huh_collector_custom extends huh_collector {
   ...
   public function keySelectCollectorAgentIDJSON($term) {
      ...
      $preparemysql = "
        SELECT agentvariant.agentid, concat(agentvariant.name,' [',case agenttype when 0 then 'Organization' when 1 then 'Individual' when 2 then 'Other' when 3 then 'Team' end, ' ',coalesce(year(agent.dateofbirth),'?'),'-',coalesce(year(agent.dateofdeath),'?'),']',' (',GROUP_CONCAT(coalesce(geography.FullName, '') SEPARATOR ', '),')') as label, agentvariant.name as value
        FROM agentvariant
...
class huh_geography_custom extends huh_geography {
   ...
   public function limitedSelectDistinctJSONGeography($country,$primary) {
      ...
      $preparemysql = "select distinct if(g1.acceptedid is null, g1.geographyid, 0-g1.geographyid), if(g1.acceptedid is null, g1.fullname, concat(g1.fullname, ' [Synonym for ',g2.fullname,']')) from geography g1 left join geography g2 on g1.acceptedid = g2.geographyid $wherebit order by g1.name, g1.fullname asc ";
      $comma = '';
      if ($stmt = $connection->prepare($preparemysql)) {
         $stmt->execute();
         $stmt->bind_result($id,$name);
